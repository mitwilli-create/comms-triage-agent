...
//    - DOC_OES_COMMS_GUIDE: Always loaded
//    - DOC_OES_HOW_WE_WORK: Always loaded
//    - DOC_CM_STRATEGY: Change Management triggers
//    - DOC_CM_COMMS_STRATEGY: Change Management triggers
//    - DOC_TRR_PLAYBOOK: Technical Roadmap triggers
//    - DOC_APPROVALS_USER_GUIDE: Mandate/governance triggers
//    - DOC_CSP_STRATEGY: Change Sequencing Platform/ROI/Systems Catalog triggers
//    - DOC_CATALOG_POR: Change Sequencing Platform/ROI/Systems Catalog triggers
//    - DOC_CATALOG_ARCH: Change Sequencing Platform/ROI/Systems Catalog triggers
//    - DOC_ROI_CHARTER: Change Sequencing Platform/ROI/Systems Catalog triggers
//    - DOC_DEPENDENCY_PRD: Change Sequencing Platform/ROI/Systems Catalog triggers
...
//   - External audiences (outside OES)
//   - Sensitive topics
...
function getConfig() {
  ...
  return {
    ...
    // Email settings
    SENDER_NAME: 'OES Communications',
    REPLY_TO: '[TEAM_EMAIL]'
  };
}
...
function getLivingDocsConfig() {
  ...
  return {
    // Always load
    always: [
      { name: 'OES Comms Guide', id: props.getProperty('DOC_OES_COMMS_GUIDE') },
      { name: 'OES Team: How We Work', id: props.getProperty('DOC_OES_HOW_WE_WORK') }
    ],
    
    // Change Management triggers: "Change Management", "change management", "mandate"
    icm: [
      { name: 'Change Management Strategy and Principles', id: props.getProperty('DOC_CM_STRATEGY') },
      { name: 'OES Change Management Communication Strategy', id: props.getProperty('DOC_CM_COMMS_STRATEGY') }
    ],
    
    // Technical Roadmap triggers: "Technical Roadmap", "technical recommendation"
    trr: [
      { name: 'OES Technical Roadmap Playbook v2', id: props.getProperty('DOC_TRR_PLAYBOOK') },
      { name: 'Technical Roadmap Principles', id: props.getProperty('DOC_TRR_PRINCIPLES') }
    ],
    
    // Mandate triggers: request_type = "Mandate/Governance" OR "approval"
    mandate: [
      { name: 'OES Approvals - User Guide', id: props.getProperty('DOC_APPROVALS_USER_GUIDE') }
    ],
    
    // Change Sequencing Platform/ROI/Systems Catalog/Dependency triggers
    cmpRoiCatalog: [
      { name: 'Change Sequencing Platform Product Strategy and Narrative', id: props.getProperty('DOC_CSP_STRATEGY') },
      { name: 'Systems Catalog POR v2', id: props.getProperty('DOC_CATALOG_POR') },
      { name: 'Systems Catalog Architecture Overview', id: props.getProperty('DOC_CATALOG_ARCH') },
    ...
    // Website triggers: request_type = "Website Update" OR "site", "wiki"
    website: [
      { name: 'OES Site Style Guide', id: props.getProperty('DOC_SITE_STYLE_GUIDE') }
    ],
    
    // Onboarding triggers: "onboarding", "new hire"
    onboarding: [
      { name: 'OES Onboarding', id: props.getProperty('DOC_ONBOARDING') }
    ]
  };
}
...
function getLivingDocs(formData) {
  ...
  if (searchText.includes('cmp') || 
      ...
      searchText.includes('catalog') ||
      searchText.includes('dependency')) {
    docsToLoad = docsToLoad.concat(LIVING_DOCS.cmpRoiCatalog);
    Logger.log('Triggered: Change Sequencing Platform/ROI/Systems Catalog/Dependency docs');
  }
  
  // Website triggers
  if (requestType.includes('website') || 
      searchText.includes('site') ||
      searchText.includes('wiki')) {
...
BEFORE:
"The Office of Engineering Strategy has upcoming major engineering decisions ready for broader feedback. This email along with the oes-announce@ and oes-approvals@ lists are part of our work to create a more predictable process for gathering company-wide feedback on key decisions. We will be sending out these announcements as often as every two weeks as needed.
...
AFTER:
"The Office of Engineering Strategy (OES) is ready to get your input on upcoming, major engineering decisions.

**Why it Matters:** We're building a more predictable process for gathering company-wide feedback. This email, along with the oes-announce@ lists, is part of that effort.
...
BEFORE:
"Faster, Simpler Systems Catalog Portal Live"

AFTER:
"Faster, Simpler Portal for Curated Infrastructure Products"

WHY THIS WORKS:
- "Systems Catalog" means nothing to readers unfamiliar with the product
...
BEFORE:
"Engineering managers average roughly 90 reports per principal IC, with wide variance, some over 200 and some under 20."
...
BEFORE:
"OES Domains Quarterly Insights - 2025 Q2 Newsletter"

AFTER:
"OES Domains Insights - 2025 Q2"
...
AFTER:
"An OES-orchestrated V2 roadmap will consolidate 30+ solutions into fewer systems, streamlining practices, improving sharing, ensuring compliance."
...
BEFORE:
"OES helped drive the Config-Push Greenlight Greenlight, accelerating convergence of configuration and data push mechanisms across the company to Cascade and Relay. We exited Greenlight on March 11, 2025."
...
/**
 * v1.3 CHANGE 3: Added site/website/wiki auto-escalation trigger
 */
const TRIAGE_PROMPT = `You are the OES Internal Comms Triage Agent. Your job is to classify incoming communications requests and route them appropriately.
...
| Audience is EXTERNAL to OES (other PAs, Platform teams not in OES, external partners) | HIGH TOUCH | escalation_agent |
...
- The context makes clear this is OUTSIDE OES (e.g., "Platform VP", "Cloud Director", "PA leadership")
...
- Audience is INTERNAL to OES (OES Leadership, OES teams, Domain Stewards, etc.)
...
- NOT related to website/site/wiki content
...
- Website/site/wiki content (requires IA/structural decisions)
- Audience is cross-org but NOT external to the company

### HIGH TOUCH (routing: "escalation_agent")
ANY of these:
- ABSOLUTE TRIGGERS ([VP_NAME], external audience, outside OES)
- Audience is EXTERNAL to OES (other PAs, outside leadership, external partners)
- Org-wide announcement outside OES
- Sensitive topic (reorg, performance, layoffs)
- Urgent + high-stakes combination
- Executive communications to audiences OUTSIDE OES
 ...
 */
const REVISION_PROMPT = `You are the OES Internal Comms revision engine.
...
### IF AUDIENCE CONTAINS: "cross-org", "outside OES", "partner teams", "Platform Engineering"
Apply CROSS-ORG rules:
- EXPLAIN OES-specific terminology
- LEAD with their benefit
- ADD context they may not have
- CLARIFY ownership

### IF AUDIENCE CONTAINS: "OES", "team", "engineers", or no specific indicators
Apply GENERAL OES TEAM rules:
...
If Primary Audience is "OES Leadership" → PROCEED with revision, do NOT return routing_error.
...

{
  ...
  "detected_audience_type": "senior_technical_ic" | "cross_org" | "general_oes" | "vp_director_requires_escalation",
  "notification_email": {
...
- CM docs → only for CM/change management requests
- Systems Catalog/CSP/ROI docs → only for those specific programs
- Domains docs → only for Domains requests
- OES-wide docs (Comms Guide, How We Work, onboarding) → reference for ANY request
...
function onFormSubmit(e) {
  ...
  try {
    ...
    // v1.3 CHANGE 3: IMMEDIATE MEDIUM TOUCH CHECK for site/website/wiki
    if (isSiteRelatedRequest(formData)) {
      ...
      const mediumTouchTriageResult = {
        ...
        routing: "escalation_agent",
        classification_reasoning: {
          primary_factors: ["Site/website/wiki content requires human review for IA decisions"],
          escalation_triggers: ["Site-Related Content Rule"],
          signals_detected: { low: [], medium: ["Website/wiki content"], high: [] }
        },
...
function isSiteRelatedRequest(formData) {
  ...
  const siteKeywords = [
    'website', 'site', 'wiki', 'wiki', 'web page', 'webpage', 
    'landing page', 'internal site', 'mini site', 'minisite',
    'the intranet', 'goto link', 'go-link', 'golink'
  ];
...
function parseFormResponse(e) {
  ...
  return {
    ...
    requester_name: '',
    team: responses['Your Team/OES Workstream:'] ? responses['Your Team/OES Workstream:'][0] : '',
...
✓ Keep to one page or less`;
  ...
  if (audience.includes('cross') || audience.includes('partner') || 
      audience.includes('platform engineering') || audience.includes('outside oes')) {
...
✓ Keep jargon to a minimum`;
  ...
  return `AUDIENCE TYPE: General OES Team
...
function sendEscalationEmail(formData, triageResult, rowNumber, starterDocUrl, config) {
  ...
  try {
    GmailApp.sendEmail(
      ...
      '',
      {
        name: config.SENDER_NAME || 'OES Communications',
...
function testHighTouchTemplateV2() {
  ...
  const testFormData = {
    ...
    subject: 'Platform Engineering org restructure announcement',
    summary: 'We need to announce the org restructure to all of Platform Engineering and communicate changes to partner teams.',
    target_audience: 'Platform Engineering (1000+), Partner PAs',
    vp_involved: 'No',
  ...
  const testTriageResult = {
    ...
    confidence: 95,
    confidence_label: 'high',
    request_summary: 'Org restructure announcement to Platform Engineering and partners',
    classification_reasoning: {
...
function testVpHighTouchTemplateV2() {
  ...
  const testFormData = {
    ...
    subject: 'Q1 OES Exchange - [VP_NAME] opening remarks',
    summary: 'Need to draft [VP_NAME]\'s opening remarks for the Q1 OES Exchange. Should cover 2025 priorities, acknowledge team efforts, and introduce the agenda.',
    target_audience: 'OES Organization (all hands)',
  ...
  const testTriageResult = {
    ...
    confidence: 100,
    confidence_label: 'high',
    request_summary: 'Draft [VP_NAME]\'s opening remarks for Q1 OES Exchange',
    classification_reasoning: {
...
function testAntiSpinDetection() {
  ...
  const testFormData = {
    ...
    target_audience: 'OES Organization',
    vp_involved: 'Yes - sent by [VP_NAME]',
    urgency: 'Within 2-3 days',
    has_draft: false
  };
...
function testUpwardCommsDetection() {
  ...
  const testFormData = {
    ...
    request_type: 'Email',
    content_status: 'Outline/rough notes only',
    subject: 'Flag: Unstaffed products process for Platform Engineering',
    summary: 'Need to draft an email from [VP_NAME] to [EXEC_NAME_2] flagging the new unstaffed products process we want to roll out to Platform Engineering Directors.',
...
function checkForBypassEmails() {
  ...
  threads.forEach(thread => {
    ...
    pingChatSpace(`🚨 **BYPASS ALERT**\n\n**From:** ${msg.getFrom()}\n**Subject:** ${msg.getSubject()}\n\nSomeone emailed ${config.REPLY_TO} directly. Redirect to the intranet`, config);
...
function testLowTouchPath() {
  const mockEvent = {
    ...
    namedValues: {
      ...
      'Primary Audience': ['OES team (~50 engineers)'],
...
function testQAFixes_v17() {
  ...
  var p1 = getRecommendedVpPrompt({ summary: 'Normal email draft', target_audience: 'OES team' });
  var p2 = getRecommendedVpPrompt({ summary: 'Unfortunately we need to delay the launch', target_audience: 'OES team' });
