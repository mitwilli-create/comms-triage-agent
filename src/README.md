...
Production Google Apps Script source for the Comms Triage Agent. Organization-specific names, programs, and tooling have been replaced with generic, fictional equivalents, and example data is synthetic. Per-deployment values use bracketed placeholders.
...
| Property | Description |
|---|---|
| `DOC_OES_COMMS_GUIDE` | Comms guide document ID (always loaded) |
| `DOC_OES_HOW_WE_WORK` | How We Work document ID (always loaded) |
| `DOC_CM_STRATEGY` | Loaded on change management triggers |
| `DOC_CM_COMMS_STRATEGY` | Loaded on change management triggers |
...
| `DOC_CSP_STRATEGY` | Loaded on program-specific triggers |
| `DOC_CATALOG_POR` | Loaded on program-specific triggers |
| `DOC_CATALOG_ARCH` | Loaded on program-specific triggers |
...
This is a sanitized public version. Organization-specific names, programs, and tooling have been replaced with generic, fictional equivalents (for example, a fictional "Office of Engineering Strategy" at "Northwind"), and all example data and knowledge-base content is illustrative rather than real.

A few per-deployment values remain as bracketed fill-ins for you to set when adapting the agent:
...
| `[EXEC_NAME]`, `[EXEC_NAME_2]` | Executive names referenced in examples |
| `[TEAM_EMAIL]`, `[EMAIL_ADDRESS]` | Team and individual email addresses |
