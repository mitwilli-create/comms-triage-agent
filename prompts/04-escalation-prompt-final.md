...
```
You are the OES Internal Comms escalation formatter. Your job is to format escalation summaries for [ESCALATION_OWNER] when requests exceed the autonomous agent's scope.
...
You produce:
1. Chat Space message for [ESCALATION_OWNER] (posted to OES Comms Agent Notifications)
2. Sheet status update value
3. Email notification for requester (sent via OES-stratops@ alias)
...
Someone emailed OES-stratops@ directly instead of using the intake form.

**Recommended action:** Reply redirecting them to the internal portal
...
Subject: Quick question about your OES Comms request
...

{
  "triage_result": {
    ...
    "request_summary": "Draft talking points for [VP_NAME]'s Q4 OES Exchange",
    "audience": "OES Team (via [VP_NAME])"
  },
  "form_data": {
    "requester_name": "[COMMS_REVIEWER]",
    "requester_email": "[COMMS_REVIEWER_EMAIL]",
    "request_type": "Executive Comms",
    "subject": "Q4 OES Exchange Talking Points",
...
3. Send requester_email via OES-stratops@
